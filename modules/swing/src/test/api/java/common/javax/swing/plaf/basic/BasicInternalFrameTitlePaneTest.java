...
public class BasicInternalFrameTitlePaneTest extends SwingTestCase {
    ...
    private void checkButtonIcons() {
        ...
        assertTrue("closeButton tooltip ok",
                pane.closeButton.getToolTipText() != null);
        assertTrue("iconButton tooltip ok",
                pane.iconButton.getToolTipText() != null);
        assertTrue("maxButton tooltip ok",
                pane.maxButton.getToolTipText() != null);
    }

    private void checkEnabledActions() {
        assertEquals("iconifyAction", !frame.isIcon() && frame.isIconifiable(),
                     pane.iconifyAction.isEnabled());
        assertEquals("closeAction", frame.isClosable(),
                pane.closeAction.isEnabled());
        ...
        assertEquals("moveAction",
                isHarmony() && frame.getDesktopPane() != null && !frame.isMaximum(),
                pane.moveAction.isEnabled());
        assertEquals("sizeAction",
                !frame.isMaximum() && frame.isResizable()
                && !frame.isIcon(),
                pane.sizeAction.isEnabled());

        assertEquals("restoreAction",
                frame.isMaximum() && frame.isMaximizable()
                || frame.isIcon() && frame.isIconifiable(),
                pane.restoreAction.isEnabled());
    }
    ...
    public void testCreateActions() {
        ...
        assertTrue("iconifyAction instanceof ok", pane.iconifyAction
                   instanceof BasicInternalFrameTitlePane.IconifyAction);

        assertTrue("maximizeAction != null", pane.maximizeAction != null);
        assertTrue("maximizeAction instanceof ok", pane.maximizeAction
                   instanceof BasicInternalFrameTitlePane.MaximizeAction);

        assertTrue("moveAction != null", pane.moveAction != null);
        assertTrue("moveAction instanceof ok", pane.moveAction
                   instanceof BasicInternalFrameTitlePane.MoveAction);

        assertTrue("restoreAction != null", pane.restoreAction != null);
        assertTrue("restoreAction instanceof ok", pane.restoreAction
                   instanceof BasicInternalFrameTitlePane.RestoreAction);

        assertTrue("sizeAction != null", pane.sizeAction != null);
        assertTrue("sizeAction instanceof ok", pane.sizeAction
                   instanceof BasicInternalFrameTitlePane.SizeAction);
    }
    ...
    public void testAssembleSystemMenu() {
        ...
        assertTrue("menuBar contains windowMenu",
                   menuBar.getMenu(0) == pane.windowMenu);
    }
    ...
    public void testTitlePaneLayout() {
        ...
        assertEquals("menuBar", menuBarBounds,
                     pane.menuBar.getBounds());
        assertEquals("iconButton", zeroBounds,
                     pane.iconButton.getBounds());
        assertEquals("maximizeButton", zeroBounds,
                     pane.maxButton.getBounds());
        assertEquals("closeButton", zeroBounds,
                     pane.closeButton.getBounds());
        ...
        assertEquals("menuBar", menuBarBounds,
                     pane.menuBar.getBounds());
        assertEquals("iconButton", closeButtonBounds,
                     pane.iconButton.getBounds());
        assertEquals("maximizeButton", zeroBounds,
                     pane.maxButton.getBounds());
        assertEquals("closeButton", zeroBounds,
    ...
    public void testPropertyChangeHandler() {
        ...
        frame.setIconifiable(true);
        assertTrue("icon button added", belongs(pane.iconButton,
                    pane.getComponents()));

        frame.setIconifiable(false);
        assertFalse("icon button removed", belongs(pane.iconButton,
                pane.getComponents()));
        ...
        frame.setClosable(true);
        assertTrue("close button added", belongs(pane.closeButton,
                pane.getComponents()));

        frame.setClosable(false);
        assertFalse("close button removed", belongs(pane.closeButton,
                pane.getComponents()));
        ...
        frame.setMaximizable(true);
        assertTrue("max button added", belongs(pane.maxButton,
                pane.getComponents()));

        frame.setMaximizable(false);
        assertFalse("max button removed", belongs(pane.maxButton,
                pane.getComponents()));
    }
