...
public class AbstractDocument_AbstractElementTest extends TestCase {
    ...
    protected static void init() {
        ...
        aElement = aDocument.new DisAbstractedElement(null,
    ...
    public void testAbstractElement02() {
        ...
        try {
            ...
            fail("Error 'Illegal cast to MutableAttributeSet' "
                 + "should be thrown");
        } catch (Error e) {
            // Do nothing
        } finally {
    ...
    private static final String dumpTextAtLimit =
        "123456789\n" +         // 10 chars / 10
        "123456789\n" +         // 10 chars / 20
        "123456789\n" +         // 10 chars / 30
        "aaabbbccc";            //  9 chars / 39 + 1 def '\n'
    private static final String dumpTextBeyondLimit =
        "123456789\n" +         // 10 chars / 10
        "123456789\n" +         // 10 chars / 20
        "123456789\n" +         // 10 chars / 30
    ...
    public void testGetAttributeThruParent() {
        ...
        assertFalse(parented.containsAttribute(StyleContextTest.attr[1],
                parented.getAttribute(StyleContextTest.attr[0])));
        // getAttribute DOES look up attributes in parent
        assertSame(StyleContextTest.attr[1],
                parented.getAttribute(StyleContextTest.attr[0]));
    }
