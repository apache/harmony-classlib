...
//        assertFalse(event1.isConsumed());
    ...
    public void testScrollRectToVisible() {
        ...
        assertEquals("rectangle's tranformed properly ", new Rectangle(40, 40, 1, 1), container4.scrollRect);
    }
    ...
    public void testGetNextFocusableComponent() {
        ...
        panel.setNextFocusableComponent(panel2);
        assertEquals("nextFocusableComponent set correctly ", panel2, panel.getNextFocusableComponent());
    }
    ...
    public void testSetVerifyInputWhenFocusTarget() {
        ...
        changeListener.checkLastPropertyFired(panel, "verifyInputWhenFocusTarget", Boolean.TRUE, Boolean.FALSE);
        changeListener.reset();

        panel.setVerifyInputWhenFocusTarget(true);
        changeListener.checkLastPropertyFired(panel, "verifyInputWhenFocusTarget", Boolean.FALSE, Boolean.TRUE);
        changeListener.reset();
    ...
    public void testSetDebugGraphicsOptions() {
        if (!isHarmony()) {
            panel.setDebugGraphicsOptions(DebugGraphics.LOG_OPTION | DebugGraphics.FLASH_OPTION);
            assertTrue("options are set correctly ", (panel.getDebugGraphicsOptions() & DebugGraphics.FLASH_OPTION) != 0);
            assertTrue("options are set correctly ", (panel.getDebugGraphicsOptions() & DebugGraphics.LOG_OPTION) != 0);
            assertFalse("options are set correctly ", (panel.getDebugGraphicsOptions() & DebugGraphics.BUFFERED_OPTION) != 0);

            panel.setDebugGraphicsOptions(DebugGraphics.BUFFERED_OPTION);
            assertFalse("options are set correctly ", (panel.getDebugGraphicsOptions() & DebugGraphics.FLASH_OPTION) != 0);
            assertFalse("options are set correctly ", (panel.getDebugGraphicsOptions() & DebugGraphics.LOG_OPTION) != 0);
            assertTrue("options are set correctly ", (panel.getDebugGraphicsOptions() & DebugGraphics.BUFFERED_OPTION) != 0);
    ...
    public void testSetDefaultLocale() {
        Locale locale = JComponent.getDefaultLocale().equals(Locale.CHINA) ?
                        Locale.ITALIAN : Locale.CHINA;
...
//        assertFalse(JComponent.isLightweightComponent(awtPanel));
    ...
    public void testSetComponentPopupMenu() {
        ...
        if (isHarmony()) {
            listener1.checkPropertyFired(panel, "componentPopupMenu",
                                         null, popup1);
        }
    ...
    private void removeListeners(final JComponent comp, final Class c) {
        ...
        for (int i = 0; i < listeners.length; i++) {
            comp.listenerList.remove(c, listeners[i]);
        }
    }
}