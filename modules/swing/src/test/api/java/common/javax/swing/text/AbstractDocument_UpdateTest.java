...
public class AbstractDocument_UpdateTest extends TestCase {
     ...
     */
    private static void checkLevelsAndBounds(final Element root,
                                             final int[] levels,
                                             final int[] bounds) {
        for (int i = 0; i < levels.length; i++) {
            ...
            assertEquals("Start offset different at " + i,
                         bounds[i], start);
            assertEquals("End offset different at " + i,
                         bounds[i + 1], end);
        }
    }
    ...
    public void testInsertUpdate08() throws BadLocationException {
        ...
        checkLevelsAndBounds(root,
                             new int[] {0, 1, 2, 0, 1, 2, 1, 0},
                             new int[] {0, 3, 6, 9, 10, 13, 19, 20, 21});
        ...
        checkLevelsAndBounds(root,
                             new int[] {0, 1, 2, 0, 1, 2, 1, 2, 1},
                             new int[] {0, 3, 6, 9, 10, 13, 19, 20, 23, 27});
        // Elements added "\n" of level 1 [19,20]
    ...
    public void testInsertUpdate11() throws BadLocationException {
        ...
        doc.insertString(0, LTR,
                         context.addAttribute(context.getEmptySet(),
    ...
    public void testRemoveUpdate09() throws BadLocationException {
        doc.insertString(0, LTR + RTL + DIG + RTL + DIG + LTR + DIG + RTL,
                         null);
        assertEquals(8, root.getElementCount());
        checkLevelsAndBounds(root,
                             new int[] {0, 1, 2, 1, 2, 0, 1, 0},
                             new int[] {0, 3, 6, 9, 12, 15, 21, 24, 25});
