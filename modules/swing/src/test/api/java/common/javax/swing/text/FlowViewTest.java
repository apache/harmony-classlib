...
public class FlowViewTest extends BasicSwingTestCase {
    ...
    protected static class FlowViewImpl extends FlowView {
        ...
        protected View createRow() {
            return new BoxView(getElement(),
                               getAxis() == Y_AXIS ? X_AXIS : Y_AXIS) {
                ...
                public int getStartOffset() {
                    return getViewCount() > 0 ? getView(0).getStartOffset()
                                              : super.getStartOffset();
                }

                public int getEndOffset() {
                    int count = getViewCount();
                    return count > 0 ? getView(count - 1).getEndOffset()
                                     : super.getEndOffset();
                }
            };
        }
    ...
    public void testGetViewIndexAtPositionEntire() {
        ...
        assertEquals(-1,
                     view.getViewIndexAtPosition(second.getEndOffset() + 2));
    ...
    public void testGetViewIndexAtPositionPartial() {
        ...
        final int middle = (first.getStartOffset() + first.getEndOffset()) / 2;
        View[] views = new View[] {
            new ElementPartView(first, first.getStartOffset(), middle),
            new ElementPartView(first, middle, first.getEndOffset()),
            vf.create(second)
        };
        view.replace(0, 0, views);
    ...
    public void testLoadChildrenStrategy() {
        ...
        view.strategy = new FlowStrategy() {
            public void insertUpdate(FlowView fv, DocumentEvent event,
                                     Rectangle alloc) {
                called[0] = true;
    ...
    public void testCalculateMinorAxisRequirements() {
        ...
        SizeRequirements xSR = view.calculateMinorAxisRequirements(View.X_AXIS,
                                                                   null);
    ...
    public void testLayout() {
        final List layoutChanges = new ArrayList();
        ...
        assertEquals(View.X_AXIS, ((Integer)layoutChanges.get(0)).intValue());
        assertEquals(View.Y_AXIS, ((Integer)layoutChanges.get(1)).intValue());
        layoutChanges.clear();
    ...
    private int getSpanY() {
        ...
        for (int i = 0; i < row.getViewCount(); i++) {
            span = Math.max(span,
                            (int)row.getView(i).getPreferredSpan(View.Y_AXIS));
        }
        return span;
    }
}
