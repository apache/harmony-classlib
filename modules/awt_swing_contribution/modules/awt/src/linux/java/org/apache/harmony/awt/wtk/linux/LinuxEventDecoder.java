...
public final class LinuxEventDecoder extends NativeEvent {
    ...
    /*Optional paramaters*/
    private ArrayList clipRects;
    private Rectangle clipBounds;
    private ArrayList accumulatedClipRects; //We ignore expose events unless
                                            // we receive a whole events' chain.
                                            // See XExposeEvent.count
    ...
    X11.XEvent exposeEvent;

    public static long getUTCOffset() {
        return utcOffset;
    }

    LinuxEventDecoder(long javaWindowId, LinuxWindowFactory factory) {
        this.javaWindowId = javaWindowId;
        accumulatedClipRects = null;
    ...
    public void setEvent(X11.XEvent event) {
        super.reset();

        windowId = event.get_xany().get_window();
        nativeEventId = event.get_type();
        /*Set to '0' all optional parameters*/
        ...
        if (windowId == javaWindowId) {
            ...
            return;
        } else if (!factory.validWindowId(windowId) &&
    ...
    private void processExposeEvent(X11.XExposeEvent event) {
        if (accumulatedClipRects == null) {
            accumulatedClipRects = new ArrayList();
        }

        accumulatedClipRects.add(new Rectangle(event.get_x(), event.get_y(),
        ...
        if (event.get_count() != 0) {
            ...
            clipRects = accumulatedClipRects;
            clipBounds = (Rectangle) accumulatedClipRects.get(0);
            for (int i = 0; i < clipRects.size(); i++) {
                clipBounds.add((Rectangle) clipRects.get(i));
            }
    ...
    private void processConfigureEvent(X11.XConfigureEvent event) {
        ...
        if (child && (windowRect.width == oldRect.width)
                && (windowRect.height == oldRect.height)) {

            exposeEvent = x11.createXEvent(false);

            X11.XExposeEvent ee = exposeEvent.get_xexpose();
            ee.set_window(windowId);
            ee.set_type(X11Defs.Expose);
            ee.set_x(0);
            ee.set_y(0);
            ee.set_width(windowRect.width);
            ee.set_height(windowRect.height);
            ee.set_count(0);
        }
    }
    ...
    private void processFrameExtentsEvent(long property) {
        ...
        //System.out.println("frame extents!id=" + windowId + ",ins=" + insets);
        //processInsetsChange(insets);
    }
