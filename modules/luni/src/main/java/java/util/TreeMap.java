...
public class TreeMap<K, V> extends AbstractMap<K, V> implements
        SortedMap<K, V>, Cloneable, Serializable {
    ...
    private static class UnboundedEntryIterator<K, V> extends
    ...
    private static class ComparatorBoundedIterator<K, V> extends
            AbstractMapIterator<K, V> {
        private final K endKey;

        private final Comparator<? super K> cmp;

        ComparatorBoundedIterator(TreeMap<K, V> map, Entry<K, V> startNode,
                K end) {
            super(map, startNode);
            endKey = end;
            cmp = map.comparator();
        ...
        @Override
        public boolean hasNext() {
            return (node != null && endKey != null)
                    && (cmp.compare(node.key, endKey) < 0);
        }
    }

    private static class ComparatorBoundedEntryIterator<K, V> extends
            ComparatorBoundedIterator<K, V> implements
    ...
    private static class ComparableBoundedIterator<K, V> extends
            AbstractMapIterator<K, V> {
        private final Comparable<K> endKey;

        public ComparableBoundedIterator(TreeMap<K, V> treeMap,
                Entry<K, V> entry, Comparable<K> endKey) {
            super(treeMap, entry);
            this.endKey = endKey;
        }
    ...
    private static class ComparableBoundedEntryIterator<K, V> extends
            ComparableBoundedIterator<K, V> implements
            Iterator<Map.Entry<K, V>> {

        ComparableBoundedEntryIterator(TreeMap<K, V> map,
                Entry<K, V> startNode, Comparable<K> end) {
            super(map, startNode, end);
        }
    ...
    static class SubMapEntrySet<K, V> extends AbstractSet<Map.Entry<K, V>>
        ...
        public Iterator<Map.Entry<K, V>> iterator() {
            ...
            if (subMap.hasEnd) {
                ...
                if (cmp == null) {
                    return new ComparableBoundedEntryIterator<K, V>(
                            subMap.backingMap, startNode,
                            toComparable(subMap.endKey));
                }
                return new ComparatorBoundedEntryIterator<K, V>(
                        subMap.backingMap, startNode, subMap.endKey);
            }
    ...
    static class SubMapKeySet<K, V> extends AbstractSet<K> implements Set<K> {
        ...
        public Iterator<K> iterator() {
            ...
            if (subMap.hasEnd) {
                Comparator<? super K> cmp = subMap.comparator();
                if (cmp == null) {
                    return new ComparableBoundedKeyIterator<K, V>(
                            subMap.backingMap, startNode,
                            toComparable(subMap.endKey));
                }
                return new ComparatorBoundedKeyIterator<K, V>(
                        subMap.backingMap, startNode, subMap.endKey);
            }
    ...
    static class SubMapValuesCollection<K, V> extends AbstractCollection<V> {
        ...
        public Iterator<V> iterator() {
            ...
            if (subMap.hasEnd) {
                Comparator<? super K> cmp = subMap.comparator();
                if (cmp == null) {
                    return new ComparableBoundedValueIterator<K, V>(
                            subMap.backingMap, startNode,
                            toComparable(subMap.endKey));
                }
                return new ComparatorBoundedValueIterator<K, V>(
                        subMap.backingMap, startNode, subMap.endKey);
            }
